...
export function ReviewPageClient({ id }: ReviewPageClientProps) {
  ...
  const canReject =
    ...
    !undoExpiresAt;
  const canApprove = canReject && !isSuperseded;
  ...
  function startHold() {
    if (!canApprove || !isAuthenticated || decisionState.status === "submitting") return;
      ...
      {showRejectSheet ? (
        <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm" onClick={() => setShowRejectSheet(false)}>
          ...
          >
            ...
            <button
              type="button"
              disabled={!canReject || decisionState.status === "submitting"}
              onClick={() => {
                if (!isAuthenticated) {
                  redirectToLogin();
                  return;
